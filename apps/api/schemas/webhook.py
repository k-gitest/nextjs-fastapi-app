...
class AnalyticsEventWebhookPayload(BaseModel):
    ...
    構造:
        event_type: イベント分類（"todo_event" | "auth_event"）
        event_data: イベント固有のデータ（dict）
            - action:        何をしたか（"created" | "updated" | "deleted"）
            - user_id:       ユーザーID（必須）
            - todo_id:       TodoID（todo_event必須）
            - priority:      優先度（todo_event任意）
            - progress:      進捗（todo_event任意）
            - timestamp:     イベント発生時刻 ISO8601（必須）
            - correlation_id: 分散トレース用ID（任意）
    """
    ...
    def validate_event_data_fields(self) -> 'AnalyticsEventWebhookPayload':
        ...
        if self.event_type == AnalyticsEventType.auth_event:
            ...
            required_fields = ['user_id', 'todo_id', 'action', 'timestamp']
            missing_fields = [f for f in required_fields if f not in self.event_data]
            if missing_fields:
                raise ValueError(
                    f"event_data is missing required fields: {', '.join(missing_fields)}"
